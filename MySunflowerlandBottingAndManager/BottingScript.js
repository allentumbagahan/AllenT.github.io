...
// Add Toggle Button
    ...
    function kitchenData(){
        ...
        else 
        {
            ...
            // find close btn
            const findClose = new Promise(function(res, err){
                for (let x=0; x<AllImage.length; x++){
                    if (AllImage[x].getAttribute("class") == "absolute cursor-pointer z-20"){
                        closeBTN = AllImage[x]
                        res('close button found')
                    }
                }
                err('no close button')
            }).then(function(val){
